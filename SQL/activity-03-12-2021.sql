...
SELECT count(distinct rental.inventory_id) as unique_inventory_rented,count(inventory.inventory_id) as total_rented 
...
where concat(actor.first_name,' ', actor.last_name) = 'Dan Torn' or 
concat(actor.first_name,' ', actor.last_name) = 'Dan Streep' ORDER BY film_title ASC;
...
from category join film_category on category.category_id =  film_category.category_id join
...
where category.name = 'Comedy' and actor.last_name like 'D%' ORDER BY actor_full_name ASC;